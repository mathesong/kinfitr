...
Loganplot <- function(t_tac, tac, input, tstarIncludedFrames, weights, inpshift = 0, vB = 0, frameStartEnd) {
  ...
  out <- list(par = par, par.se = par.se, fit = logan_model, tacs = tacs, 
...
plot_Loganfit <- function(loganout, roiname = NULL) {
  ...
  plotdf <- data.frame(Weights = loganout$weights,
  ...
  outplot <- ggplot(data=plotdf, aes(x=Logan_Plasma, y = Logan_ROI, colour=Equilibrium)) + 
    ...
    geom_abline(slope     = as.numeric(loganout$fit$coefficients[2]), 
