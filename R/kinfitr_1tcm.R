...
plot_1tcmfit <- function(onetcmout, roiname = NULL) {
  ...
  if( onetcmout$inpshift_fitted ) {
    mod_tac_start <- head(onetcmout$fit$m$getEnv()$t_tac)[-1]
    dat_tac_start <- head(onetcmout$tacs$Time)[-1]
  ...
  fitdf <- data.frame(
    Time = onetcmout$input$Time + tacshift,
    Radioactivity = i_fit,
