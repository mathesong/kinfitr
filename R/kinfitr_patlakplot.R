...
Patlakplot <- function(t_tac, tac, input, tstarIncludedFrames, weights, inpshift = 0, frameStartEnd) {
  ...
  out <- list(par = par, par.se = par.se, fit = patlak_model, tacs = tacs, 
              fitvals = fitvals, input = input, weights = weights,
              inpshift = inpshift, tstarIncludedFrames = tstarIncludedFrames)
  return(out)
...
plot_Patlakfit <- function(patlakout, roiname = NULL) {
  ...
  plotdf <- data.frame(Weights = patlakout$weights,
  ...
  if(roiname != 'ROI') { plotdf = plyr::rename(plotdf, c('ROI_measured' = roiname) ) }
  ...
  outplot <- ggplot(data=plotdf, aes(x=Patlak_Plasma, y = Patlak_ROI, colour=Equilibrium)) + 
    ...
    geom_abline(slope     = as.numeric(patlakout$fit$coefficients[2]), 
