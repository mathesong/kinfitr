...
SIME <- function(t_tac, tacdf, input, Vndgrid, weights, roiweights, 
  ...
  if(!missing(twotcmstart)) {
    twotcmout <- twotcm(t_tac, tac = tacdf[,twotcmstart], input, weights=weights, inpshift, vB)
    k2_pars$start <- twotcmout$par$k2
    k3_pars$start <-  twotcmout$par$k3
    k4_pars$start <-  twotcmout$par$k4
  ...
  Cost <- tidytacs %>% 
    ...
    dplyr::do(gridCost = SIMEroi(t_tac=.$Time, tac=.$Radioactivity, input=input, Vnd=.$Vnd[1],
                                 vB=vB, weights = .$weights,
              k2.start = k2_pars$start , k2.lower = k2_pars$lower , k2.upper = k2_pars$upper ,
              k3.start = k3_pars$start , k3.lower = k3_pars$lower , k3.upper = k3_pars$upper ,
              k4.start = k4_pars$start , k4.lower = k4_pars$lower , k4.upper = k4_pars$upper)) %>%
...
SIMEroi <- function(t_tac, tac, input, Vnd, vB, weights,
                    k2.start = 0.1 , k2.lower = 0 , k2.upper = 0.5 ,
                    k3.start = 0.1 , k3.lower = 0 , k3.upper = 0.5 ,
                    k4.start = 0.1 , k4.lower = 0 , k4.upper = 0.5 ) {
  ...
  pred <-   tryCatch( {
    minpack.lm::nlsLM(tac ~ SIME_model(t_tac, input, Vnd, k2, k3, k4, vB),
                      start =  c(k2 = k2_pars$start, k3 = k3_pars$start, k4 = k4_pars$start),
                      lower = c(k2 = k2_pars$lower, k3 = k3_pars$lower, k4 = k4_pars$lower),
                      upper = c(k2 = k2_pars$upper, k3 = k3_pars$upper, k4 = k4_pars$upper),
