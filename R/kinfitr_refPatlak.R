...
refPatlak <- function(t_tac, reftac, roitac, tstar, weights = NULL,
...
refPatlak_tstar <- function(t_tac, reftac, lowroi, medroi, highroi, filename = NULL, frameStartEnd = NULL, timeStartEnd = NULL, gridbreaks = 2) {
  ...
  fitfunc <- function(roitac, tstar) {
    refPatlak(t_tac, reftac, roitac, tstar = tstar, frameStartEnd = frameStartEnd)
  }

  comp <- tstar_compute(t_tac, lowroi, medroi, highroi, fitfunc,
                        function(f) f$par$Kiref, tstarInclFrames)
  ...
  totalplot <- tstar_finalise_plot(
    ...
    outcome_ylab = expression(K[i]^{ref}), t_tac = t_tac,
    tstarInclFrames = tstarInclFrames, gridbreaks = gridbreaks,
    outcome_ylim = NULL, filename = filename, modelname = "refPatlak"
  )
