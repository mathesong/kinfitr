...
mlLoganplot <- function(t_tac, tac, input, tstar, weights = NULL,
...
mlLogan_tstar <- function(t_tac, lowroi, medroi, highroi, input, filename = NULL, inpshift = 0, vB = 0, frameStartEnd = NULL, timeStartEnd = NULL, gridbreaks = 2) {
  ...
  fitfunc <- function(roitac, tstar) {
    mlLoganplot(t_tac, roitac, input, tstar = tstar, inpshift = inpshift, vB = vB, frameStartEnd = frameStartEnd)
  }
  ...
  totalplot <- tstar_finalise_plot(
    ...
    outcome_ylim = NULL, filename = filename, modelname = "mlLoganplot"
  )
