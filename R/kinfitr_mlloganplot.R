...
mlLoganplot <- function(t_tac, tac, input, tstarIncludedFrames, weights, inpshift = 0, frameStartEnd) {
  ...
  fitvals <- data.frame(Time = t_tac, Weights = weights,
...
plot_mlLoganfit <- function(loganout, roiname = NULL) {
  ...
  plotdf <- data.frame(Weights = refmlloganout$weights,
...
mlLogan_tstar <- function(t_tac, lowroi, medroi, highroi, input, filename, inpshift = 0, frameStartEnd, gridbreaks=2) {
  ...
  highroi_fit <- mlLoganplot(t_tac, highroi, input, tstarIncludedFrames = frames, inpshift = inpshift, frameStartEnd = frameStartEnd)
  ...
  for(i in 1:length(tstarInclFrames)) {
    ...
    lowfit  <- mlLoganplot(t_tac, lowroi, input, tstarIncludedFrames = tstarInclFrames[i], inpshift = inpshift, frameStartEnd = frameStartEnd)
    medfit  <- mlLoganplot(t_tac, medroi, input, tstarIncludedFrames = tstarInclFrames[i], inpshift = inpshift, frameStartEnd = frameStartEnd)
    highfit <- mlLoganplot(t_tac, highroi, input, tstarIncludedFrames = tstarInclFrames[i], inpshift = inpshift, frameStartEnd = frameStartEnd)
